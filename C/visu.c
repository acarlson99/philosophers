...
void draw_state(SDL_Renderer *renderer, SDL_Texture *circle_texture,
	...
	for (int ii = 0; ii < num; ++ii) {
		...
		SDL_RenderCopyEx(renderer, philo_texture, NULL,
						 ...
						 philos[ii].dead || philos[ii].state != none ? 0
																	 : shake,
						 NULL, SDL_FLIP_NONE);
		...
		SDL_RenderFillRect(
			renderer, &(SDL_Rect){health_x, health_y + (HBAR_HEIGHT - height),
								  HBAR_WIDTH, height});

		// plate
		SDL_SetTextureColorMod(circle_texture, PLATE_COLOR);
		float plate_x = philos[ii].x * PLATE_CENTER_OFF + WINDOWWIDTH / 2 -
						(PHILO_SIZE / 2);
		float plate_y = philos[ii].y * PLATE_CENTER_OFF + WINDOWHEIGHT / 2 -
						(PHILO_SIZE / 2);
